...
const otherFiles = [...htmlFiles(DIST)].filter(
  (file) => !postFiles.includes(file)
);
...
describe("BlogPosting structured data", () => {
  ...
  it("emits exactly one BlogPosting on every post — all of them, not a sample", () => {
    const wrong = postFiles
      .map((file) => ({
        ...
        count: (
          fs.readFileSync(file, "utf-8").match(/"@type":"BlogPosting"/g) ?? []
        ).length,
      }))
      .filter((p) => p.count !== 1);
  ...
  it("emits none anywhere else, including the blog listing", () => {
    const leaked = otherFiles
      .filter((file) =>
        fs.readFileSync(file, "utf-8").includes('"@type":"BlogPosting"')
      )
  ...
  it("parses, and carries the required fields with no empty values", () => {
    for (const file of postFiles) {
      ...
      for (const key of [
        ...
        expect(String(article[key]), `${page} → ${key}`).not.toMatch(
          /undefined|null/
        );
      }
      ...
      expect(article.image, page).toBe(
        `https://mcp-hangar.io/og/blog/${slug}.png`
      );
  ...
  it("leaves the site-wide SoftwareApplication block intact on posts", () => {
    for (const file of postFiles) {
      const page = path.relative(DIST, file);
      const types = jsonLdBlocks(fs.readFileSync(file, "utf-8")).map(
        (o: any) => o["@type"]
      );
  ...
  it("names the same publisher the product block names as author", () => {
    ...
    const app: any = jsonLdBlocks(html).find(
      (o: any) => o["@type"] === "SoftwareApplication"
    );
