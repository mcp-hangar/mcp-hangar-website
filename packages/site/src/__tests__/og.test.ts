...
describe("ogPathFor", () => {
  it("maps the sections a card is generated for", () => {
    ...
    expect(ogPathFor("/blog/2026-07-16-security-advisory-cve-2026-59950")).toBe(
      "/og/blog/2026-07-16-security-advisory-cve-2026-59950.png"
    );
    expect(ogPathFor("/docs/adr/ADR-009-independent-release-topology")).toBe(
      "/og/docs/adr/ADR-009-independent-release-topology.png"
    );
  ...
  it("ignores a trailing slash", () => {
    ...
    expect(ogPathFor("/learn/more-than-one-hangar/")).toBe(
      "/og/learn/more-than-one-hangar.png"
    );
  });
...
describe("fitTitle", () => {
  it("drops a size as titles get longer", () => {
    const sizes = [
      "CLI",
      "Deploying the operator to a shared cluster today",
  ...
  it("never returns a string longer than the smallest tier allows", () => {
    const long =
      "Configuring the enforcement plane for multi-tenant clusters where each namespace carries its own egress policy and the operator reconciles admission webhooks across a shared PostgreSQL backed control plane with fencing";
