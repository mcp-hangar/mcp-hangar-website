...
describe("blogPostingSchema", () => {
  ...
  it("points image at the post’s own generated OG card, absolutely", () => {
    expect(blogPostingSchema(base).image).toBe(
      "https://mcp-hangar.io/og/blog/a-post.png"
    );
  });
  ...
  it("types a team byline as an Organization and a name as a Person", () => {
    ...
    expect(
      blogPostingSchema({ ...base, author: "Marcin Pyrka" }).author
    ).toEqual({
      "@type": "Person",
      name: "Marcin Pyrka",
    });
...
describe("toJsonLd", () => {
  ...
  it("round-trips a full schema through JSON.parse", () => {
    expect(JSON.parse(toJsonLd(blogPostingSchema(base)))["@type"]).toBe(
      "BlogPosting"
    );
  });
});
