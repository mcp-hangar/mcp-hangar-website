...
describe("per-page OG cards", () => {
  ...
  it("gives every indexable page its own card, never the site-wide fallback", () => {
    const fellBack = indexable
      .filter((p) => p.og === "/og-image.png")
      .map((p) => p.page);
  ...
  it("renders every card at 2400×1260", () => {
    ...
    for (const card of cards) {
      expect({ card, ...pngSize(path.join(DIST, card)) }).toEqual({
        card,
        ...EXPECTED,
      });
    }
  ...
  it("keeps twitter:image on the per-page card too", () => {
    ...
    expect(home).toContain(
      'name="twitter:image" content="https://mcp-hangar.io/og/home.png"'
    );
  });
});
