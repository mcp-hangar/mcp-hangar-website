...
<Callout variant="preview">
  **Landing in 2.0.** The gate described here ships on the **v2 preview**
  (`2.0.0rc3`, built on the stable `mcp==2.0.0`) and is **not** in the released `1.6.2`
  line, which still rejects any upstream task handle with
  `TaskRelayNotSupported`. On the preview it is live by default;
