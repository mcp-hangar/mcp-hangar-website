...
<Callout variant="preview">
  **Landing in 2.0, and off by default.** The gate described here ships on the
  **v2 preview** (`2.0.0rc2`, built on `mcp==2.0.0b2`) and is **not** in the
  released `1.6.2` line, which still rejects any upstream task handle with
  `TaskRelayNotSupported`. On the preview it is opt-in: `relay_tasks_enabled`
  defaults to **false**.
