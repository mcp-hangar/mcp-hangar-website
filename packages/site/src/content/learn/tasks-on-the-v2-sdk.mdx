...
The v2 SDK changes that fact. In `mcp==2.0.0` -- the reshape SEP-2663
...
2026-07-28 removed from the core spec. Measured across `mcp==2.0.0` and
`2.0.0rc1`, thirteen days apart: all 29 `Task*` classes are field-for-field
identical, while the module around them was edited. A deliberately frozen region
of a live file, not an API still settling -- so the probes could never trip.
`ListTasksResult` exists forever, so `tasks/list` was always served;
`UpdateTaskRequest` never arrives, so `tasks/update` never was.
...
and the SDK v2 migration (`mcp==2.0.0`, SEP-2663 Tasks). Everything is MIT and
