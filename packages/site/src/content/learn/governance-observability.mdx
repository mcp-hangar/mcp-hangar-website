...
description: "OTel GenAI/MCP semconv spans and an identity-attributed audit record exported to OTLP and your SIEM over every governed call."
type: Concept
level: Core
time: "6 min"
theme: observability
...
real time, and an audit record attributed to the identity that made the call,
exported to your telemetry and SIEM backends rather than kept only for the
lifetime of a trace.
...
audit record answers *what happened, who did it, and under which contract* --
...
## An attributed audit record, not a log line

Spans expire, and they sample. Audit records do neither. Security-relevant
domain events -- `ToolInvocationCompleted`, `ToolInvocationFailed`,
`McpServerStateChanged` -- are exported as OTLP log records whenever
`OTEL_EXPORTER_OTLP_ENDPOINT` is set, with no extra configuration, and to your
SIEM in the formats below. Retention is your collector's, which means it is
governed by the policy you already run for security telemetry.

What that does *not* yet include is a durable event stream inside Hangar
itself. Aggregate state is persisted as state, not as an append-only chain, so
the guarantee on offer today is delivery and attribution rather than local
replay. The event-sourced spine is being wired now, deliberately and in the
open; until it is, this page describes what ships.
...
Identity attribution only matters if the record reaches the
...
One thing you will not find on a Hangar span or in its audit record is an
...
in the audit record says *which* rule denied it. Egress policy is
...
audit record names who made it and under what rule; both speak conventions
