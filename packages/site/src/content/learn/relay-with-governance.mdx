...
<Callout variant="note">
  **Shipped in 2.0.0.** Everything on this page describes the governed task
  relay as released in **2.0.0**. It is **not** in 1.6.x, which still cleanly
...
A client follows up on a relayed task through three native
...
Everything that is not a confirmed answer leaves the gate shut. Walk the
branches of the inbound `tasks/update`:

- **The caller never declared the Tasks extension**, or does not own the task.
  Refused before the gate is touched — an unattributed caller is fail-closed by
  the same ownership rule the other two handlers use.
- **The client says nothing.** There is no prompt to decline, because Hangar
  never asks. A task left in `input_required` stays there; silence is not
  consent and never becomes one.
- **The relay of the answer fails upstream.** The gate is discarded *without*
  being consumed and the call raises `task update relay failed; retry`. This one
  is deliberately **recoverable**, not terminal: a retry re-drives the same
  update rather than burning the consent.
...
The ordering is the point: the gate opens *before* the payload is relayed and is
consumed only once the upstream confirms, so there is no window in which the
answer reaches the upstream without a recorded decision behind it.
...
task in the safe direction and leaves an attributed line in the provenance record. It
...
lifecycle transition of a relayed task emits an event keyed on `task_id`,
...
names a `TaskInputRequired` transition in the lifecycle set, but on 2.0.0
