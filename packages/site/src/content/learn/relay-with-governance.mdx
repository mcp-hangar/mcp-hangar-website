...
An MCP task breaks that. `tasks/create` returns almost immediately, but it
returns a *promise*: a `task_id` and the assurance that the real work -- a long
report, a multi-minute crawl, a job that needs a human to say yes halfway
through -- will finish later. The result arrives on a `tasks/result` you make
minutes or hours after the call that started it.
...
call across minutes. Between `tasks/create` and `tasks/result`, the upstream tool
...
TaskInputRequired    # paused, awaiting consent
...
The full lifecycle of any relayed task is reconstructable from that stream. This
is the same forensic non-repudiation the synchronous call path already gives you
...
The 2026-07-28 protocol handshake and the [SEP-2663](https://modelcontextprotocol.io/)
Tasks reshape change *how* mid-flight input is resolved: the modern surface drives
