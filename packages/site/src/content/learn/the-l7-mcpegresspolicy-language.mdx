...
invocation instant to go find one -- so it resolves to a distinct require-approval
verdict that fails closed (blocks) today, and the call stops. It is enforcement,
not a hold. Read `requireApproval` as *"this class of call is refused unless it has
been approved out of band,"* not as *"pause here and ask."* Routing a gated call
into an interactive approval workflow is itself a planned follow-up, not shipped
behaviour -- which is exactly why the shipped require-approval verdict simply
blocks (it is its own outcome, `REQUIRE_APPROVAL`, not a deny).
