...
Registration answers one question -- *may this server receive traffic at all?* In a
namespace opted into enforcement the operator applies a default-deny egress,
admission validates the server's spec, and a registered server's egress is
withheld until its image is digest-pinned. That is a switch, not a policy. `MCPEgressPolicy` is the
