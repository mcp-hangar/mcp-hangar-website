---
title: "From install to a governed deny with an operator and a cluster"
description: "Install the operator into a Kubernetes cluster, apply a policy, and watch the same call go from allowed to denied."
section: tutorials
updated: 2026-07-22
...
**Before you start:** this walkthrough needs a Kubernetes cluster you can
`kubectl apply` to, Hangar core already running in it, an `MCPServer` registered,
and Cilium if any upstream is a hostname rather than an IP. It is the
cluster-scale version of the story. If you want the same lesson on a laptop in
five minutes, read [From install to a governed deny in 60 seconds (no
cluster)](/learn/from-install-to-a-governed-deny-locally) instead — same
mechanism, one machine, no operator.
...
operator-side controller that delivers it end-to-end, shipped in operator
**`v0.14.0`** — point one policy at one server, and make the
...
- The Hangar **core** (`1.6.2` or newer) running and reachable in the cluster --
  this is the data plane the L7 rules are enforced on, and whose front door you
  will call. The version is a floor, not a pin: everything below works on every
  release since.
...
- **Installed the plane** — operator (`v0.14.0`+) plus core (`1.6.2`+), wired
