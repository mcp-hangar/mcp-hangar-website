...
`MCPEgressPolicy` plane that ships in **1.6.0** (core `1.6.0`, operator
`v0.14.0`), point one policy at one server, and make the *exact same* tool call
succeed and then get denied -- with nothing changing but the policy.
...
sits in the middle and **fails closed** -- a gated call is *blocked* pending an
out-of-band decision. It is a hard gate, not an interactive approval queue: the
call does not hang waiting for a human to click yes, it stops.
...
[ADR-013](/adr/ADR-013-egress-policy-enforcement-model) (the enforcement model,
