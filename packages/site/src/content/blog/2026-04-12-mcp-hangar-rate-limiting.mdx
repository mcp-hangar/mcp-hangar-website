...
```mermaid
flowchart TD
    client["MCP client"] --> bus["CommandBus.send(command)"]
    bus --> mw["RateLimitMiddleware.__call__()"]
    mw --> consume["rate_limiter.consume(key)"]

    consume -->|allowed| next["next handler"]
    consume -->|rejected| reject["HTTP 429<br/>+ X-RateLimit headers<br/>+ Retry-After"]
```
