...
```mermaid
flowchart TD
    invoke["MCP client invokes tool"] --> check["ToolAccessResolver.check(tool_name, policy)"]

    check -->|deny_list match| block["BLOCK immediately"]
    check -->|"allow_list match (or no rule)"| exec["EXECUTE"]
    check -->|approval_list match| pending["CREATE pending approval"]

    pending --> notify["notify channel<br/>dashboard / Slack / noop"]
    notify --> wait["wait for decision<br/>up to approval_timeout_seconds"]

    wait -->|approved| exec
    wait -->|denied| reject["REJECT with reason"]
    wait -->|timeout| err["ERROR<br/>error_code: approval_timeout"]
```
