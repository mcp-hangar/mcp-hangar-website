...
This post is about waking it up. It's **landing in 2.0**, running today on the v2 preview (`2.0.0a1`, `mcp==2.0.0b2`) -- not in released 1.6.0, not GA. Here's what the seam does, why it was closed until now, and the one thing it unblocks: the first interactive, mid-flight consent gate on the governed call path.
...
To be exact about the truth of it: this is **on the v2 preview** (`2.0.0a1`, `mcp==2.0.0b2`), **landing in 2.0**. It is not in released 1.6.0 and it is not GA. Released Hangar today is still 1.6.0.
