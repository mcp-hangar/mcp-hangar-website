...
export async function ogCards(): Promise<Card[]> {
  ...
  const cards: Card[] = [
    { slug: "home", page: "/", props: { kind: "home" } },
    {
      ...
      props: {
        kind: "blog",
        title: "Engineering notes from the enforcement plane",
      },
    },
    {
      slug: "learn",
      ...
      props: {
        kind: "learn",
        title: "Understand how Hangar governs every MCP call",
      },
    },
    {
      slug: "security",
      ...
      props: {
        kind: "security",
        title: "Security",
        note: "CVE ledger · OWASP MCP Top 10",
      },
    },
    {
      slug: "docs",
  ...
  for (const e of learn.filter((e) => !e.data.draft)) {
    cards.push({
      slug: `learn/${e.id}`,
      page: `/learn/${e.id}`,
      props: { kind: "learn", title: e.data.title },
    });
  }
  ...
  for (const e of security) {
    cards.push({
      slug: `security/${e.id}`,
      page: `/security/${e.id}`,
      props: { kind: "security", title: e.data.title },
    });
  }
