  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-100 overflow-hidden">
      ...
      <div className="relative">
        ...
        <div className="max-w-6xl mx-auto px-6 pb-32">
          <div className="mb-6">
            <h2 className="text-sm font-semibold text-zinc-500 uppercase tracking-wider mb-2">Quick Start</h2>
            <p className="text-zinc-400">From install to first tool call in under a minute.</p>
          </div>
          <TerminalAnimation />
        </div>
...
const terminalLines = [
  { text: '$ curl -sSL https://get.mcp-hangar.io | bash', type: 'command', delay: 30 },
  { text: '✓ mcp-hangar installed (v0.3.0)', type: 'success', delay: 0 },
  { text: '', type: 'empty', delay: 0 },
  { text: '$ mcp-hangar init', type: 'command', delay: 40 },
  { text: 'Created config.yaml with example provider', type: 'output', delay: 0 },
  { text: '', type: 'empty', delay: 0 },
  ...
  { text: '', type: 'empty', delay: 0 },
  { text: '$ mcp-hangar invoke math add \'{"a": 2, "b": 3}\'', type: 'command', delay: 25 },
...
function TerminalAnimation() {
  ...
  const getLineClassName = (type: string) => {
    switch (type) {
      case 'command': return 'text-zinc-100'
      case 'success': return 'text-emerald-400'
      case 'output': return 'text-zinc-400'
      case 'status': return 'text-emerald-400'
      case 'result': return 'text-amber-400 font-semibold'
  ...
  const renderLine = (line: { text: string; type: string }, index: number) => {
    ...
    if (line.type === 'command') {
      return (
        <div key={index} className={getLineClassName(line.type)}>
          <span className="text-emerald-500">$</span>{line.text.slice(1)}
        </div>
      )
    }
    ...
    if (line.type === 'status') {
      return (
        <div key={index} className={getLineClassName(line.type)}>
          <span className="text-emerald-400">●</span>{line.text.slice(1)}
        </div>
      )
    }
  ...
  const currentLine = terminalLines[currentLineIndex]
  const typingText = currentLine?.delay > 0 ? currentLine.text.slice(0, currentCharIndex) : ''
      ...
      <div className="p-6 font-mono text-sm min-h-[280px]">
        ...
        {isTyping && currentLine?.delay > 0 && (
          <div className="text-zinc-100">
            <span className="text-emerald-500">$</span>{typingText.slice(1)}
